# This module implements the user profile management section, where specialists can view and update their personal data entered during registration. 
# For example, if a specialist needs to recover their password, they can use this section to set a new one after recovery.
...
# Window for viewing and editing the specialist's personal profile information.
# Allows the specialist to update fields such as password, address, and contact details.

class SpecialistProfile(ctk.CTkToplevel):
    def __init__(self, parent, user_id, role, full_name, on_profile_update=None):
        super().__init__(parent)
        self.transient(parent)
        self.lift()
        self.focus_force()
        self.parent = parent  # Used to update the dashboard after changes
        self.user_id = user_id
        self.state('zoomed')
        self.title("Specialist Profile")
        self.geometry("600x700")
        self.configure(fg_color="#E8F5E9")
        self.role = role
        self.full_name = full_name
        self.on_profile_update = on_profile_update  # Callback to update parent if needed
        self.data = {}
        self.entries = {}
        self.editing = False
    ...
    def load_user_data(self):
        """Load the specialist's data from the database and store it in self.data."""
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        cursor.execute("""
            SELECT ID_Person, Name, Surname, Birthplace, Birthdate, Street, City, ZIP, TaxCode, Email, Password, Telephone
            FROM Person WHERE ID_Person = ? AND Role = 'Specialist'
        """, (self.user_id,))
        row = cursor.fetchone()
        conn.close()
    ...
    def save_data(self):
        ...
        # Validate telephone
        if not re.match(r"^\+?\d+$", new_data["Telephone"]):
            self.message_label.configure(text="Telephone must contain only numbers (optional '+').", text_color="red")
            return

        # ZIP must be digits only
        if not new_data["ZIP"].isdigit():
            self.message_label.configure(text="ZIP must contain only digits.", text_color="red")
            return
