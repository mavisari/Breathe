# This module implements the notifications screen where the patient receives their notifications.
# Notifications can be of three types:
# - Calendar: when the doctor books, reschedules, or deletes an appointment.
# - Assistance: when a ticket is resolved by the technician.
# - Contact: when the patient receives a message from a specialist.
# Clicking on a notification redirects the patient to the related page.
...
# Window to visualize patient notifications.
# Displays system notifications and recent chat messages from specialists.
# Clicking a notification marks it as read and redirects to the related page.

class PatientNotifications(ctk.CTkToplevel):
    ...
    def load_notifications_from_db(self):
        """Load notifications and recent chat messages from the database for the current user.
        ...
        try:
            ...
            # Load system notifications for the user ordered by most recent
            c.execute("""
            ...
            for row in rows:
                ...
                # Only allow expected notification types; classify unknown as 'other'
                if notif_type not in ("Calendar", "Contact", "Assistance"):
                    notif_type = "other"

                # Prepare display text with emoji icons based on notification type
                if notif_type == "Calendar":
                    display_text = f"📅 {title}: {body}"
                elif notif_type == "Contact":
                    display_text = f"📞 {title}: {body}"
                elif notif_type == "Assistance":
                    display_text = f"🛠️ {title}: {body}"
                else:
                    display_text = f"{title}: {body}"
            ...
            # Load last 20 chat messages received from specialists
            c.execute("""
                SELECT 
                    cm.ID_Message, 
                    cm.SenderID, 
                ...
                WHERE cm.ReceiverID = ? AND cm.ID_Message LIKE "MS%"
                AND cm.SenderID IN (SELECT ID_Specialist FROM Specialist)
            ...
            # Sort all notifications by date and time descending (most recent first)
            from datetime import datetime as dt

            def sort_key(x):
                dt_str = f"{x['date']} {x['time']}"
                try:
    ...
    def on_notification_click(self, notification, status_circle):
        ...
        if self.master:
            if notification["type"] == "Calendar" and hasattr(self.master, "open_patient_calendar"):
                self.master.open_patient_calendar()
            elif notification["type"] == "Contact" and hasattr(self.master, "open_contact_specialist"):
                self.master.open_contact_specialist()
